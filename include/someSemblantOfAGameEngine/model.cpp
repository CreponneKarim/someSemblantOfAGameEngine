...
Model::Model(std::string path,glm::mat4 model){
	Model::loadModel(path);
	Model::model=model;
}
...
bool Model::getIsALight(){
	return Model::isALight;
}
void Model::switchLight(){
	Model::isALight=!(Model::isALight);	
}
...
void Model::loadModel(std::string path){
	...
	Model::modelName=path.substr(path.find_last_of(PATH_SEPARATOR),path.size());
...
unsigned int Model::TextureFromFile(const char str[],std::string path){
...
int Model::textureExists(const char str[])const{
